...
目前要回答的问题包括：
\begin{itemize}
	\item Transformer 做不到某些 逻辑语法上的运作，问题出在哪里？
	
	\item Na\"{i}ve rule base 似乎存在旧有的逻辑学习算法的问题，亦即 ``plateau problem''.
	
	\item 可能存在 介乎 Transformer 与 naive rule base 之间的算法，它比 Transformer 更接近逻辑，但比 naive rule base 更快。 但这跟解决 第一个问题好像没有关系。
\end{itemize}
