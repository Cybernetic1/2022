...
\end{equation}
等 $2 \times 3 = 6$ 个命题 也是同构的。 但前者是由两组不同的概念结合而成，它的成分可以被 $\forall$ 或 $\exists$ 量化； 后者是 命题逻辑，命题是不可分割的，也不可以拆开来量化。 
...
\begin{condition}[AC]
	Each proposition $P$ is made up of $K$ atoms:
	\begin{equation}
	P_i = a_{i1} \cdot ... \cdot a_{iK}
	\label{eqn:atomic-condition-1}
	\end{equation}
	where optionally some atoms are \textbf{copied} to other locations (with a non-linear transformation $\tau$, if they are copied to the output layer) via:
	\begin{equation}
...
\textbf{Self-Attention} 的本质 可以这样理解：
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=1]{essence-of-self-attention.png}}}
\end{equation}
这垂直的「軸」结构 ({\color{red}红色})，重复在每一个轴上。  所以，当 输入的元素 \textbf{交换}时，输出也随着交换。 这就是 self-Attention 能达到 \textbf{equi-variant} 效果的原因。
