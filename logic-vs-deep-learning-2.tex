...
This is the most basic operation of classical logic AI:
}
\begin{equation}
...
For example: \logic{human(Socrates)} can unify with the left side of the formula. \\
}
\cc{Matching 的结果是得到一推 instantiated（特例化，即不包含变量）的命题。
}{
The result of Matching is to get a proposition that is instantiated (= specialized, ie, does not contain variables).
}
...
The special thing about deep learning is that it imitates the inference process
}
\begin{equation}
...
with a very complex non-linear function (ie, a deep neural network).  After doing so, the logic rules are ``mingled'' together so that it's hard to tell them apart.  But it is precisely because of this ``intertwining'' that the deep neural network compresses a huge number of combinatorial logic rules into a smaller number of layer-by-layer parameters.  It performs both learning and inference.  This simple and crude method is actually extremely efficient, and it is not easy to surpass its speed!
}
...
We know (or speculate) that an intelligent system should possess the structure of symbolic logic. Can this knowledge be used to constrain/accelerate deep neural networks? The answer seems to be positive. The current state-of-the-art CNN (for vision) and GPT (for language) both have specialized internal structure, \textbf{rather than fully-connected}, and this internal structure corresponds to the structure of the data being processed. Therefore, we have reason to believe that logical structure can be used to constrain the structure of the deep neural network to achieve acceleration.
}
...
Next, let's look at the structure of logical systems in detail.
}

\cc{Knowledge Base 里面有很多 rules，系统要将这些 rules 逐一 match with 系统状态 (= working memory) 里面的命题：
}{
There are many rules in the Knowledge Base, and the system needs to match these rules one by one against propositions in the system's state (= working memory):
}
\begin{equation}
...
Successfully matched rules can derive new conclusions and add them back to the state / working memory.
}

\cc{这个复杂的操作，完全被一个神经网络取代。 或者可以更抽象地说：
}{
This complicated process is completely replaced by a neural network.  Or more abstractly:
}
\begin{equation}
...
For the Transformer, it is a kind of memory stored \textbf{between} input elements (stored in the $Q, K, V$ matrices), and it \textbf{implicitly} achieves the role of logic rules:
}
\begin{equation}
...
In other words, there is some sort of (distorted) logic rules inside the Transformer.  The natural question then is:  Can we find additional structures of logic / logic systems?  That is, what kind of algebraic structure constrains (\ref{eqn:some-kind-of-memory})?  For this question, we can take insights from categorical logic and classical logic-based AI systems.
}
...
We wish to formulate algebraic constraints for (\ref{eqn:some-kind-of-memory}), but for now it is easier to describe in words:
}
\begin{itemize}
\cc{\item 状态是 \textbf{颗粒化} 的，它是某集合的元素，元素之间可交换，也就是 Transformer 的 equivariance. （注意： Transformer 有 equivariance，但 equivariance 未必一定要用 Transformer 实现）
	...
	\item The state is \textbf{granular}, made of elements of a certain set, and such elements can be permuted, which corresponds to the equivariance of the Transformer.  (Note: Transformers are equivariant, but equivariance does not necessarily require Transformers)
}

\cc{\item \textbf{深度结构}：例如多层网络，每层是函数的复合 (composition). Transformer 也用了深度结构。
}{
	\item \textbf{Deep structure}:  Such as multi-layer networks, each layer is a composition of functions. Transformer also has a deep structure.
}
...
\cc{\item 逻辑 包括了 \textbf{命题}层次 和 \textbf{命题内部}层次 的 颗粒化。 后者是\textbf{谓词} (predicate) 逻辑的结构，例如： \logic{loves( John, Mary )}，也可以简单地将它视为 \textbf{代数元}之间的\textbf{乘积}，例如： \logic{John \bullet\  loves \bullet\   Mary}, 后者也叫做 ``word''.  （不同类别的代数元之间不一定容许乘积，因此有 groupoid 的概念，但暂时来说这细节不重要。） 现时重点是如何将 这两层的 颗粒化 结构 施加到深度神经网络上。 
	...
	\item Logic has granularity at the \textbf{proposition} level and at the \textbf{sub-propositional} level.  The latter is the structure of \textbf{predicate} logic, for example: \logic{loves( John, Mary )} can be regarded as the \textbf{product} of \textbf{algebraic elements}, as in: \logic{John \bullet\ loves \bullet\ Mary}, the latter is also called a ``word''.  The details are unimportant.  Our focus is on how to apply this 2-level granular structure to deep neural networks.
}
...
\cc{\item 逻辑推导 每步只产生\textbf{一个}新的结论（或其概率分布），然后这个新的结论，再加入到旧的状态中，作为一个命题集合的元素，而旧状态也要 \textbf{遗忘} 一些命题，否则需要无限记忆。 这跟 Transformer 每次输出\textbf{一列}的 tokens 有点不同（虽然我们不太肯定 Transformer tokens 究竟对应于 命题 还是 谓词／原子概念）。 
	...
	\item Each step of logic inference produces \textbf{one} new conclusion (or its probability distribution), and this new conclusion is added to the old state as an element of a proposition set, and the old state also needs to \textbf{forget} some old propositions, otherwise infinite memory is required. This is a bit different from the Transformer which always outputs the \textbf{same number} of tokens as its input. (We are also unsure whether Transformer tokens correspond to propositions or to predicates / atomic concepts).
}
...
\cc{\item 逻辑 rule 通常只跟某几个前提有关，其它前提是\textbf{无关}的，例如： \logic{眼睛好看 $\wedge$ 鼻子好看 $\wedge$ 嘴巴好看 $\Rightarrow$ 帅}，跟 \logic{有钱} 或 \logic{穷} 无关。 Transformer 的 \textbf{softmax} 结构似乎也可以排除一些无关的 tokens 的影响。
	...
	\item A logic rule usually depends on some premises while other premises are \textbf{irrelevant};  For example: \logic{talk $\wedge$ dark $\wedge$ handsome $\Rightarrow$ attractive to women}, where \logic{rich} or \logic{poor} is irrelevant.  The Transformer's \textbf{softmax} also seems able to exclude some irrelevant tokens.
}

\cc{\item （可能还有其他的结构特征.....）
}{
	\item (There are possibly other structural features.....)
}
\end{itemize}
...
In my theory, the ideal logical structure is something like this (the numbers of various elements may vary):
}
\begin{equation}
...
Compared to figure (\ref{fig:logic-symmetry-propositional-only}), figure (\ref{fig:logic-symmetry-predicate-level}) is additionally constrained by $\atom$ structure.  \uline{But how can we express this constraint algebraically}?
}
...
It does not represent propositions explicitly, but it uses a special ``stop'' token to signify the \textbf{end} of sentences, and there are other ``tricks'' such as \textbf{positional encoding}.  It seems to be a rather \textit{ad hoc} design, and we should be able to improve it.
}
...
But since $\atom \in$ a non-commutative free group (the group with the least structure), it does not have a simple symmetry formula like $a \cdot b = b \cdot a$.
}
...
In fact, $\tau$ only needs to be a continuous function to meet the above requirement.  So the essence of the Atomic Condition lies in the two \textbf{equations} (\ref{eqn:atomic-condition-1}) and (\ref{eqn:atomic-condition-2}), which are actually very simple.  The category-theoretic description of $\forall$ and $\exists$ as adjoint functors is complicated, and we will explain them in  \hyperlink{page:appendix-A}{appendix A}.
}

\cc{那么 等式 (\ref{eqn:atomic-condition-2}) 里面的 ``='' 是来自哪里？ 其实太明显了，它就是逻辑 rule 里面 将变量「syntactically 搬动」的动作：
}{
So where does the ``='' in the equation (\ref{eqn:atomic-condition-2}) come from?  In fact, it is too obvious, it is just the action of syntactically ``moving'' variables in a logic rule:
}
\vspace{0.5cm}
\begin{equation}
...
Here is a very important point\footnote{Thanks ``Ziyu'' for telling me this important information. }: The precursor of Self-Attention is the \textbf{content-addressable memory} from Graves \textit{et al}'s ``Neural Turing Machines''.  So we have good reason to regard Self-Attention as a form of \textbf{memory}.
}

\cc{我们要比较 两种做法，前者是简单直接的经典 rule base 结构，后者是以 self-Attention 代替 rule base：
}{
We want to compare two approaches.  One is the na\"{i}ve classical rule-base structure, and the other uses Self-Attention instead of a rule base:
}
\begin{equation}
\label{fig:rule-base}
...
The premise of this rule has two clauses, the variable $Y$ that appears twice must be identical ({\color{red}red}), for this matching to be considered successful.  And this kind of \textbf{comparison} operation within the premises of the rule is exactly what Self-Attention can perform conveniently.  But Self-Attention also ignores the symmetry of $A \wedge B$, so there may be room for improvement.
}
...
\cc{范畴论大师 Lawvere 发现，$\forall$ 和 $\exists$ 是一个所谓 \textbf{weakening functor} 的「伴随函子」，所谓 weakening 就是从一个只有 $X$ 变量的\textbf{论域} 扩充到有 $X,Y$ 两个变量的论域，而这里 $Y$ 纯粹是一个 \textbf{dummy} variable:
	...
	The category theory master Lawvere found that $\forall$ and $\exists$ are a so-called \textbf{weakening functor} "accompanying functor", the so-called weakening is to expand from a \textbf{discourse domain} with only $X$ variables to There is a domain of two variables $X,Y$, and here $Y$ is purely a \textbf{dummy} variable:
}
\begin{equation}
...
\cc{例如 $\mbox{Love}(Y,X)$ 是带有两个变量的逻辑式子，但 $\forall Y. \mbox{ Love}(Y,X)$ 其实没有了 $Y$ 这个变量，因为它被 $\forall$ \textbf{绑定}了。
	...
	For example, $\mbox{Love}(Y,X)$ is a logical expression with two variables, but $\forall Y. \mbox{ Love}(Y,X)$ actually does not have the variable $Y$, Because it is $\forall$ \textbf{bound}.
}
...
\cc{所谓 \textbf{伴随} (adjoint) 的意思是： 有两个范畴：左边和右边。 你可以将 左边的东西搬到右边，在右边的范畴里做「\textbf{比较}」，而这个比较 同样地可以 把东西搬到左边做，两个比较是\textbf{等价}的。 这里「比较」的意思就是范畴内的 morphism, 例如在 $\mathsf{Set}$ 范畴里，比较就是 set inclusion.  
	...
	The so-called \textbf{adjoint} (adjoint) means: There are two categories: left and right. You can move things from the left to the right, and do "\textbf{comparison}" in the category on the right, and this comparison can also move things to the left, and the two comparisons are \textbf{equivalent}. Here "comparison" means morphism within a category, for example in the category $\mathsf{Set}$ comparison is set inclusion.
}
