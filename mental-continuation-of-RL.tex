...
\cc{
	...
	In other words, the reward of getting an apple back-propagates to the action of reaching out an arm for the apple.  So far so good.  But we can continue this process back to the \textbf{chain of thoughts} that decided to reach for an apple:
}
\begin{equation}
...
\cc{
	...
	So how does RL learn logical content?  An \textbf{action} here is a transition from one logic state to another; or an \textbf{inference} from some propositions to a new proposition, ie, a logic \textbf{rule}.  We need to choose from among many actions (logic rules) the best ones.  In other words, choose the best logic rule(s) from all the logic rules that are applicable to the current state.  For example, ``I'm hungry $\rightarrow$ need to find food'' is a correct rule; ``hungry $\rightarrow$ eat feces'' would be a rather bad one.
}

\cc{
	而 强化学习 的好处是： 理论上，它可以在无限的思维空间里 学到非常\textbf{抽象}的 rules，情形就像它在复杂的游戏\textbf{迷宫}里，学到破解的方法。
}{
	Advantage of doing this:  Theoretically, RL can learn highly \textbf{abstract} rules from the vast mental space, just like it learns to find solutions out of complicated \textbf{mazes} in Atari games.
}
...
\cc{
	...
	In classical AI, the \textbf{combinatorial} search of logic rules has been studied, with search trees like below: (This is however not the same as the shape of mental space, which I should definitely spend more time to characterize...)
}
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=0.5]{induction1.png}}}
\end{equation}
...
\cc{
	...
	This can be extended to our understanding of ``other minds'':
}
\begin{equation}
...
\cc{
	...
	However, in the mixed or ``flat'' view, both internal and external states belong to the same state space, within which, the internal states ``model'' the external states:\footnote{The notation $T \models M$ means: $M$ is a \textbf{model} of $T$;  $T$ is a \textbf{theory} of $M$. This is rigorously defined in \textbf{model theory} in logic.}
}
\begin{equation}
...
\cc{
	...
	Does the mixed state space has its own ``theory''?  That may be some kind of \textbf{meta-logic}.
}
