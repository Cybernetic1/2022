...
First, in the Transformer, each token has an ``axis'' for which it plays the role of the \textbf{Query}.  We can think of each token as one proposition, and each token gives rise to one new proposition along that axis.  So the Transformer layer takes $N$ input propositions and spits out $N$ new propositions:
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=1]{Transformer_axis.png}}}
\end{equation}
In the logic view, however, we may want to break a proposition into multiple tokens.

Secondly, in the Transformer, the Attention mechanism determines the \textbf{weights} by which the tokens combine, by summing up into a final token vector.  Because of ``softmax'', the new token is effectively a \textbf{selection} of one token from amongst input tokens.  In the logic view, a proposition is represented by multiple tokens, so we may need an alternative Attention mechanism to \textbf{move} tokens around (This ability is already present in the current Transformer).
...
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=0.9]{Transformer_memory-store.png}}}
\end{equation}
