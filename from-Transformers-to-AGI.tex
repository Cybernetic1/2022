...
Secondly, in the Transformer, the Attention mechanism determines the \textbf{weights} by which the tokens combine, by summing up into a final token vector.  In other words, the new token is a weighted sum of old tokens.  In the logic view, a proposition is represented by multiple tokens, so we may need an alternative Attention mechanism to \textbf{move} tokens around (This ability is already present in the current Transformer).
...
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=1]{Transformer_memory-store.png}}}
\end{equation}
